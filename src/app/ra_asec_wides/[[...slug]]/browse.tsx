...
import { getRisksAggASECWide, updateRiskAggASECWide, getSelectOptions } from "@/services";
...
export default function Page() {
  ...
  const columns: ColumnsType<DataType> = [
      ...
      children: [
        ...
        },
        {
          ...
          render: (riskCategoryL3: string, record: any) => {
            ...
            return <Link href={`/ra_asec_wides/show?${params}`}>{riskCategoryL3}</Link>
          }
        },
      ]
    },
    ...
    {
      title: 'Top Risk',
      dataIndex: 'AscPriority',
      key: 'AscPriority',
      width: 140,
      render: (AscPriority: string, _, index: number) => {
        const options = [
  ...
  React.useEffect(() => {
    ...
    if (quarter && department) {
      fetchRisksAggASECWides({fiscalYear, quarter, department});
    }
  ...
  const fetchRisksAggASECWides = (option?: RiskAggregationFilter) => {
    setLoading(true)
    getRisksAggASECWide(option)
    .then((response: any) => {
      if (response) {
        setRisks(response);
        response?.data?.forEach((risk: any, index: number) => {
          form.setFieldsValue({
            [`AscPriority[${index}]`]: ['Yes', 'No'].includes(risk.AscPriority) ? risk.AscPriority : 'Not Assessed',
  ...
  const onSubmit = async () => {
    ...
    const data = risks?.data?.map((risk: any, index: number) => {
      return {
        ...
        ascPriority: values[`AscPriority[${index}]`],
        aggRemarks: values[`aggRemarks[${index}]`],
      };
    });
    
    updateRiskAggASECWide({
      risks: data
    })
    .then(() => {
      Swal.fire({
  ...
  const onChangeCycle = (quarter: string) => {
    ...
    if (option.quarter && option.department) {
      fetchRisksAggASECWides({ ...option, fiscalYear});
    }
  };
  ...
  const onChangePageSize = (page: number, pageSize: number) => {
    ...
    fetchRisksAggASECWides({ ...option, pageSize }); 
  }
  ...
  const items: MenuProps['items'] = [
    {
      ...
      onClick: () => {
        if (Helper.getInstance().hasPermission('RISK.CREATE.NEW')) {
          router.replace('/risks/create?back=ra_asec_wides')
        }
      }
    }
  ];
