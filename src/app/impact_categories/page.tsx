...
import { ImpactCategoryInterface, ImpactCategoryFilter, } from "@/types";
import { getImpactCategory, deleteImpactCategoryById } from "@/services";
...
const Page = () => {
    if (Helper.getInstance().isRetricAccess('IMPACT.CATEGORY.VIEW')) {
        return <NoPermission />;
    }
    ...
    const fetchImpactCategories = (option?: ImpactCategoryFilter) => {
        getImpactCategory(option)
            .then((response: any) => {
                if (response?.data) {
    ...
    const deleteById = (id: number) => {
        ...
        .then((response: any) => {
            if (response?.error === 'RECORD.LINKED') {
                Swal.fire({
                    icon: "error",
                    title: "This impact category linked to risk inherent",
    ...
    const onChangePageSize = (page: number, pageSize: number) => {
        ...
        fetchImpactCategories({ ...option, pageSize }); 
    }

    const columns: ColumnsType<ImpactCategoryInterface> = [
        {
            title: 'Name',
            dataIndex: 'name',
            key: "name",
            ...
            render: (id: number, record: any) => (
                <div className="render-icon">
                    <Button
                        type="primary"
                        onClick={() => {
                            if (Helper.getInstance().hasPermission('IMPACT.CATEGORY.UPDATE')) {
                                ref.current?.open(id)
                            }
                        }}
                    ...
                    <ConfirmDelete
                        ...
                        permission={{
                            restrict: true,
                            noPermission: Helper.getInstance().isRetricAccess('IMPACT.CATEGORY.DELETE')
                        }}
    ...
    return (
        <>
            <Row gutter={[16, 16]}>
                <Col sm={24} md={6} xl={6}>
                    <PageTitle title='Impact Categories' />
                </Col>
                ...
                <Col sm={24} md={18} xl={18} style={{ display: 'flex', justifyContent: 'flex-end' }}>
                    ...
                    <Button type="primary" icon={<PlusOutlined />} onClick={() => {
                        if (Helper.getInstance().hasPermission('IMPACT.CATEGORY.CREATE')) {
                            ref.current?.open()
                        }
                    }}>
                        Add New
                    </Button>
                    <DepartmentForm reload={fetchImpactCategories} ref={ref} />
                </Col>
                ...
                <Col sm={24} md={24} xl={24}>
                    <Table
                        ...
                        onChange={(pagination, filters, sorter: any) => {
                            if (sorter.order) {
                                fetchImpactCategories({ sortColumn: sorter.field, sortType: sorter.order });
                            }
