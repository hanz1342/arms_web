...
import { getDivisions, deleteDivisionById } from "@/services";
...
export default function Page() {
  if (Helper.getInstance().isRetricAccess('DIVISION.VIEW')) {
    return <NoPermission />;
  }
  ...
  const [divisions, setDivisions] = React.useState<PaginateResponse>(new PaginateResponse());
  ...
  const fetchData = (option?: DivisionFilter) => {
    getDivisions(option)
    .then((response: any) => {
        if (response?.data) {
  ...
  const deleteById = (id: number) => {
    ...
    .then((response: any) => {
        if (response?.error === 'RECORD.LINKED') {
            Swal.fire({
                icon: "error",
                title: "This division linked to other risk",
      ...
      render: (id: number, record: any) => (
          <div className="render-icon">
              <Button
                  type="primary"
                  onClick={() => {
                    if (Helper.getInstance().hasPermission('DIVISION.CREATE')) {
                      ref.current?.open(id)
                    }
                  }}
              ...
              <ConfirmDelete
                  ...
                  permission={{
                      restrict: true,
                      noPermission: Helper.getInstance().isRetricAccess('DIVISION.DELETE')
                  }}
